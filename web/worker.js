...
self.onmessage = function(e) {
    ...
    try {
        ...
        const progressive = 1;
        const trellis = 1;        // ✅ Activado
        const trellis_dc = 1;     // ✅ Activado
        const tune_ssim = 1;      // ✅ Activado
        const optimize_scans = 1; // ✅ Activado

        const resultStructPtr = Module._compress_image(
            inputPtr, imageBuffer.byteLength, quality, 
            progressive, trellis, trellis_dc, tune_ssim, optimize_scans
        );

        const dataPtr = heapU8[resultStructPtr] | (heapU8[resultStructPtr + 1] << 8) | 
                       (heapU8[resultStructPtr + 2] << 16) | (heapU8[resultStructPtr + 3] << 24);
        const size = heapU8[resultStructPtr + 4] | (heapU8[resultStructPtr + 5] << 8) | 
                    (heapU8[resultStructPtr + 6] << 16) | (heapU8[resultStructPtr + 7] << 24);
